...
import { type GraphQLDataSourceRequestKind } from '@apollo/gateway/dist/datasources/types';
  ...
  buildService: ({ url }) =>
    new RemoteGraphQLDataSource<Apollo.Context>({
      ...
      willSendRequest: ({ kind, request: { http }, context }) => {
        if (
          kind ===
          ('incoming operation' as GraphQLDataSourceRequestKind.INCOMING_OPERATION)
        ) {
